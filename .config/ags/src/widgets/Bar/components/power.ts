import type GLib from "types/@girs/glib-2.0/glib-2.0";
import type Box from "types/widgets/box";
import type Button from "types/widgets/button";
import type Icon from "types/widgets/icon";
...
export function PowerWidget() {
	return Widget.Box({
		...
		child: Content(),
		setup: (self) =>
			self.hook(STATE.show, () =>
				self.toggleClassName("sys-mng-active", STATE.show.value),
			),
	});
}
