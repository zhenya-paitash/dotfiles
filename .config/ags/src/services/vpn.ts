...
export const VPN_ENABLED_ICON = vpn_status
	.bind()
	.as((on) => `network-vpn${on ? "" : "-disabled"}-symbolic`);

export const VPN_ENABLED_TOGGLE = () => {
	// Utils.exec(`sh -c "bluetoothctl power ${vpn_status.value ? 'off' : 'on'}"`);
	Utils.exec(
		`sh -c "nmcli connection ${vpn_status.value ? "down" : "up"} ${WIREGUARD_CONNECTION_NAME}"`,
	);
	vpn_status.value = !vpn_status.value;
};

export const SHOW_CONNECTIONS = () =>
	Utils.execAsync(`kitty nmcli connection show`);
